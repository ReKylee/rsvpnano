...
Arduino_ESP32SPI gBus(WaveshareC6TouchLcd147::DisplayWiring::kDcPin,
...
void setBacklight(bool on) { gBacklight = on; writeBacklight(); }
void setBrightness(uint8_t percent) {
    gBacklightDuty = Board::Backlight::dutyFromPercent(percent, kMinimumBacklightDuty);
    writeBacklight();
}
void sleep() { setBacklight(false); gPanel.displayOff(); }
void wake() { gPanel.displayOn(); setBacklight(true); }
