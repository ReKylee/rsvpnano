...
namespace {
    ...
    bool parseNumericEntity(const String& entity, uint32_t& codepoint) {
        ...
        for (; index < entity.length(); ++index) {
            const int digit = base == 16 ? AsciiText::hexValue(entity[index])
    ...
    bool namedEntityCodepoint(const String& entity, uint32_t& codepoint) {
        ...
        static constexpr NamedEntity kNamedEntities[] = {
            ...
            {"thorn", 0x00FE},  {"yuml", 0x00FF},   {"iexcl", 0x00A1},  {"iquest", 0x00BF}, {"copy", 0x00A9},
            {"reg", 0x00AE},    {"deg", 0x00B0},    {"plusmn", 0x00B1}, {"sup2", 0x00B2},   {"sup3", 0x00B3},
            {"sup1", 0x00B9},   {"frac14", 0x00BC}, {"frac12", 0x00BD}, {"frac34", 0x00BE}, {"laquo", 0x00AB},
            {"raquo", 0x00BB},  {"middot", 0x00B7}, {"bull", 0x2022},   {"times", 0x00D7},  {"divide", 0x00F7},
        };
    ...
    bool matchesIgnoreCaseAt(const String& text, size_t index, const char* needle) {
        for (size_t i = 0; needle[i] != '\0'; ++i) {
            if (index + i >= text.length() || AsciiText::toLower(text[index + i]) != AsciiText::toLower(needle[i])) {
                return false;
            }
        }
        return true;
...
RssFeedManager::Result RssFeedManager::checkFeeds(const OtaUpdater::Config& wifiConfig, Preferences& preferences,
...
bool RssFeedManager::fetchUrl(const String& url, String& body, String& error, uint8_t feedIndex, uint8_t feedCount,
                              StatusCallback callback, void* context) {
    ...
    for (uint8_t redirectCount = 0; redirectCount <= kMaxFeedRedirects; ++redirectCount) {
        ...
        while (http.connected() || stream->available()) {
            ...
            while (completeItemsRead < kMaxItemsPerFeed && advancePastCompleteFeedItem(body, completeItemSearchStart)) {
                ++completeItemsRead;
            }
...
bool RssFeedManager::processFeed(const String& feedUrl, const String& feedBody, Preferences& preferences,
...
bool RssFeedManager::saveItem(const feedparser::FeedItem& item, Preferences& preferences, Result& result) {
...
void RssFeedManager::report(StatusCallback callback, void* context, const String& line1, const String& line2,
                            int progressPercent) {
    if (callback == nullptr) {
        return;
    }
