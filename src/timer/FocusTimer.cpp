...
bool FocusTimer::available() const {
    return imuAvailable_;
}

bool FocusTimer::isActiveTimerRunning() const {
...
FocusTimer::Genre FocusTimer::genre() const {
    return genre_;
}
...
uint8_t FocusTimer::completedTouchBlocks() const {
    return completedTouchBlocks_;
}

uint8_t FocusTimer::completedWorkBlocks() const {
    return completedWorkBlocks_;
}

uint8_t FocusTimer::completedBreakBlocks() const {
...
bool FocusTimer::initImu() {
    ...
    for (uint8_t i = 0; i < sizeof(candidateAddresses); ++i) {
        ...
        uint8_t whoAmI = 0;
        if (!Board::Imu::readRegister(imuAddress_, kImuWhoAmIReg, whoAmI) || whoAmI != kImuWhoAmIValue) {
            Serial.printf("[timer] QMI8658 WHOAMI mismatch addr=0x%02X got=0x%02X "
                          "expected=0x%02X\n",
        ...
        if (!updateRegister(kImuCtrl1Reg, 0x40, 0x40) || !Board::Imu::writeRegister(imuAddress_, kImuCtrl8Reg, 0x80)
            || !Board::Imu::writeRegister(imuAddress_, kImuCtrl2Reg, 0x16) || !updateRegister(kImuCtrl5Reg, 0x07, 0x07)
    ...
    if (sawRespondingAddress) {
        ...
        Serial.printf("[timer] QMI8658 not responding bus=%s configured=0x%02X "
                      "fallback=0x6A\n",
...
void FocusTimer::startMode(TimerMode mode, uint32_t nowMs, uint32_t durationMs, OrientationState startOrientation) {
    activeMode_ = mode;
...
bool FocusTimer::isShortSide(OrientationState orientation) {
    return orientation == OrientationState::ShortSideA || orientation == OrientationState::ShortSideB;
}

FocusTimer::OrientationState FocusTimer::oppositeShortSide(OrientationState orientation) {
    switch (orientation) {
...
ui::Orientation FocusTimer::portraitOrientationForShortSide(OrientationState orientation) {
    return orientation == OrientationState::ShortSideB ? ui::Orientation::PortraitFlipped : ui::Orientation::Portrait;
}
