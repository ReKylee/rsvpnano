...
constexpr std::array<uint16_t, kColorRoleCount> kDefaultThemeColors = {
    0x0000, 0xFFFF, 0x8410, 0x528A, 0xF800, 0xF800, rgb565(68, 132, 88), 0xFFFF,
    0x0000, 0x2104, 0x4208, 0x8410, 0x8410, 0xF800, 0x8410, 0x8410,
};
...
bool parseColor(const String &value, uint16_t &out) {
  if (value.length() == 7 && value[0] == '#') {
    ...
    uint8_t green = 0;
    uint8_t blue = 0;
    if (!parseHexByte(value, 1, red) || !parseHexByte(value, 3, green) ||
        !parseHexByte(value, 5, blue)) {
      return false;
    }
...
String stripBom(String line) {
  if (line.length() >= 3 && static_cast<uint8_t>(line[0]) == 0xEF &&
...
int colorRoleIndexForName(const String &name) {
  ...
  const auto found = std::find_if(begin, end, [&key](const char *roleName) { return key == roleName; });
  if (found == end) {
    return -1;
  }
