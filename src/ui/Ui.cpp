...
Context::Context(Arduino_GFX& gfx, Flush flush, FlushRegion flushRegion)
    : gfx_(gfx), flush_(flush), flushRegion_(flushRegion) {}
...
bool Context::pollTouch(uint32_t nowMs) {
    touchPending_ = false;
    if (touchSource_.read == nullptr) return false;

    const Orientation orientation = touchSource_.orientation == nullptr
        ? Orientation::Portrait : touchSource_.orientation();
    ...
    if (nowMs < touchBackoffUntilMs_
        || nowMs - touchLastPollMs_ < touchSource_.timing.pollIntervalMs) return false;
...
bool Context::tab(Rect rect, std::string_view text, bool active) {
    ...
    if (widget.changed) {
        ...
        drawText(rect, text, 1,
                 color(active ? ui::themes::ColorRole::Foreground : ui::themes::ColorRole::Muted), true);
    }
    return tapped(widget.index, rect);
}
...
void Context::progress(Rect rect, int value, int minimum, int maximum) {
    ...
    if (maximum > minimum && rect.w > 2 && rect.h > 2) {
        const int16_t fill = static_cast<int16_t>((static_cast<int32_t>(rect.w - 2) * (value - minimum)) /
                                                  (maximum - minimum));
...
SliderResult Context::slider(Rect rect, int value, int minimum, int maximum, int step) {
    ...
    const bool started = event != nullptr && hasTouch(*event, TouchStart)
                      && contains(rect, event->x, event->y);
    if (started && slot < kSlotCapacity) {
        capturedSlot_ = slot;
    }

    SliderResult result{std::clamp(value, minimum, maximum), false};
    const bool moving = event != nullptr
                     && (hasTouch(*event, TouchStart) || hasTouch(*event, TouchMove)
                         || hasTouch(*event, TouchRelease));
    ...
    if (claim(Kind::Slider, rect, state).changed) {
        ...
        const int16_t knobX = maximum == minimum ? rect.x
            : static_cast<int16_t>(rect.x + (static_cast<int32_t>(rect.w - 1) * (result.value - minimum)) /
                                               (maximum - minimum));
        gfx_.fillCircle(knobX, static_cast<int16_t>(rect.y + rect.h / 2), 7,
                        color(ui::themes::ColorRole::Accent));
    }
...
void Context::drawText(Rect rect, std::string_view text, uint8_t textSize, uint16_t textColor, bool centered) {
    ...
    const int16_t x = centered ? std::max<int16_t>(rect.x, static_cast<int16_t>(rect.x + (rect.w - textWidth(text, textSize)) / 2))
                               : rect.x;
...
bool Context::tapped(size_t slot, Rect rect) {
    const Touch* event = touch();
    if (event == nullptr || slot >= kSlotCapacity) return false;
    if (hasTouch(*event, TouchStart) && contains(rect, event->x, event->y)) capturedSlot_ = slot;
    if (!hasTouch(*event, TouchRelease) || capturedSlot_ != slot) return false;
    capturedSlot_ = kSlotCapacity;
...
TouchContact Context::mapTouch(TouchContact contact) const {
    ...
    case Orientation::LandscapeFlipped: return {true, static_cast<uint16_t>(maxY - rawY), rawX};
    case Orientation::PortraitFlipped:
        return {true, static_cast<uint16_t>(maxX - rawX), static_cast<uint16_t>(maxY - rawY)};
    case Orientation::Landscape: return {true, rawY, static_cast<uint16_t>(maxX - rawX)};
    default: return {true, rawX, rawY};
    }
}

bool Context::updateTouch(const TouchContact& contact, uint32_t nowMs) {
    if (!contact.touched) {
        if (!touchActive_ || ++touchEmptySamples_ < touchSource_.timing.releaseConfirmSamples) return false;
