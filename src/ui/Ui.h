...
public:
    ...
    const Touch* touch() const { return touchPending_ ? &touchEvent_ : nullptr; }
    ...
    int16_t width() const { return gfx_.width(); }
    int16_t height() const { return gfx_.height(); }
...
private:
    enum class Kind : uint8_t { Label, Button, Tab, Progress, Slider, Dial, Custom };
