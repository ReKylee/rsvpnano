...
public:
    ...
    void setFont(const AlphaFont* font) { font_ = font; }
    ...
    int16_t textWidth(const String& text) const { return textWidth(text.c_str()); }
    ...
    int16_t textAdvance(const String& text) const { return textAdvance(text.c_str()); }
...
private:
    ...
    bool drawGlyphsToStrips(const char* text, int16_t x, int16_t baseline, const Bounds& bounds) {
        ...
        for (int16_t stripY = visibleY0; stripY < visibleY1;) {
            const uint8_t stripRows = static_cast<uint8_t>(
                std::min<int16_t>(MaxStripRows, static_cast<int16_t>(visibleY1 - stripY)));
    ...
    void compositeGlyphIntoStrip(const GlyphMetrics& glyph, int16_t glyphX, int16_t glyphY, int16_t stripX,
        ...
        for (int16_t dstY = overlapY0; dstY < overlapY1; ++dstY) {
            ...
            const uint8_t* packedRow = font_->bitmap + glyph.bitmapOffset + static_cast<uint32_t>(srcY) * glyph.rowStride;
    ...
    void flushStrip(int16_t stripX, int16_t stripY, uint16_t stripWidth, uint8_t stripRows) {
        for (uint8_t row = 0; row < stripRows; ++row) {
            ...
            while (x < stripWidth) {
                ...
                if (spanWidth > 0) {
                    // Pass mutable RAM pointer intentionally: Arduino_GFX selects the bulk writePixels() path.
    ...
    void drawPackedRowFromSpans(const GlyphMetrics& glyph, uint8_t row, int16_t dstX, int16_t dstY, int16_t displayW,
                                int16_t displayH) {
        ...
        const uint8_t* packedRow = font_->bitmap + glyph.bitmapOffset + static_cast<uint32_t>(row) * glyph.rowStride;
    ...
    static uint8_t coverageAt(const uint8_t* packedRow, uint8_t x) {
        ...
        return (x & 1U) == 0 ? static_cast<uint8_t>(packed >> 4U)
                             : static_cast<uint8_t>(packed & 0x0FU);
    }
    ...
    void rebuildBlendTable() {
        ...
        for (uint8_t coverage = 0; coverage < 16; ++coverage) {
            ...
            blend_[coverage] = static_cast<uint16_t>((static_cast<uint16_t>(r) << 11U) |
                                                     (static_cast<uint16_t>(g) << 5U) | b);
        }
    ...
    static bool nextCodepoint(const char*& text, uint16_t& codepoint) {
        ...
        auto isContinuation = [](uint8_t value) { return (value & 0xC0U) == 0x80U; };
        ...
        if ((first & 0xF0U) == 0xE0U) {
            ...
            const uint16_t value = static_cast<uint16_t>(((first & 0x0FU) << 12U) | ((b1 & 0x3FU) << 6U) |
                                                         (b2 & 0x3FU));
