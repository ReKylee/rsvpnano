...
struct Font {
    enum class Kind : uint8_t { Gfx, Alpha4 };
...
public:
    ...
    bool begin() { return alpha_.begin(); }
    void setFont(Font font) { font_ = font; alpha_.setFont(font.alpha4); }
    ...
    int16_t draw(const char* text, int16_t x, int16_t baseline) {
        if (font_.kind == Font::Kind::Alpha4) return alpha_.drawString(text, x, baseline);
        prepareGfx();
    ...
    int16_t drawCodepoint(uint16_t codepoint, int16_t x, int16_t baseline) {
        if (font_.kind == Font::Kind::Alpha4) return alpha_.drawCodepoint(codepoint, x, baseline);
        char encoded[4] = {};
    ...
    int16_t glyphAdvance(uint16_t codepoint) const {
        if (font_.kind == Font::Kind::Alpha4) return alpha_.glyphAdvance(codepoint);
        char encoded[4] = {};
        encode(codepoint, encoded);
    ...
    int16_t advance(const char* text) const {
        if (font_.kind == Font::Kind::Alpha4) return alpha_.textAdvance(text);
