...
void DisplayScreen::begin(ui::Context& ui, Preferences& preferences, size_t standbyDurationCount,
    ...
    if (setBrightness != nullptr) setBrightness(static_cast<uint8_t>((config.brightnessIndex + 1U) * 5U));

    themes.loadFromSd();
    const String savedThemeId = settings::load<pref::ThemeId>(preferences);
    if (!savedThemeId.isEmpty()) themes.selectById(savedThemeId);
    ui.setTheme(themes.selected());
}

void DisplayScreen::draw(ui::Context& ui, Preferences& preferences,
                         std::span<const uint32_t> standbyDurations, void (*setBrightness)(uint8_t),
                         Screen& screen) {
    ui::Column column{detail::content(ui), 5};
    if (ui.button(column.next(22), "Back")) screen = Screen::Settings;
    ...
    if (const auto brightness = ui.slider(column.next(28), brightnessPercent, 5, 100, 5); brightness.changed) {
        ...
        if (setBrightness != nullptr) setBrightness(static_cast<uint8_t>(brightness.value));
    }
