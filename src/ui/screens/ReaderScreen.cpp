...
bool nextCodepoint(const String& text, size_t& index, uint16_t& codepoint) {
    ...
    auto next = [&](uint8_t& value) {
        if (index >= text.length() || !continuation(static_cast<uint8_t>(text[index]))) return false;
    ...
    while (index < text.length() && continuation(static_cast<uint8_t>(text[index]))) ++index;
    codepoint = '?';
    return true;
}

bool wordCodepoint(uint16_t codepoint) {
    return codepoint < 0x80U ? LatinText::isWordCharacter(static_cast<uint8_t>(codepoint))
...
void BatteryState::update(uint32_t nowMs, bool force) {
    if (!force && nowMs - lastSampleMs < kBatterySampleMs) return;
    lastSampleMs = nowMs;
...
void ReaderScreen::begin(Preferences& preferences, uint32_t nowMs) {
    ...
    config.fontSizeIndex = settings::load<settings::prefs::ReaderFontSizeIndex>(preferences, FontCatalog::sizeCount());
    ...
    if (savedTypefaceId.isEmpty() || !fonts.indexForId(savedTypefaceId, config.typefaceIndex)) {
        config.typefaceIndex = settings::load<settings::prefs::ReaderTypefaceIndex>(preferences,
                                                                                   fonts.typefaceCount());
    }
...
bool ReaderScreen::openBook(ui::Context& ui, StorageManager& storage, Preferences& preferences, size_t index,
                            uint32_t nowMs) {
    if (!storage.mounted() || index >= storage.bookCount()) return false;
...
void ReaderScreen::loadInitialBook(ui::Context& ui, StorageManager& storage, Preferences& preferences,
    ...
    if (!savedPath.isEmpty()) {
        ...
        if (savedBook >= 0 && openBook(ui, storage, preferences, static_cast<size_t>(savedBook), nowMs)) return;
        Serial.printf("[reader] saved book not found: %s\n", savedPath.c_str());
    }
    if (storage.bookCount() > 0 && openBook(ui, storage, preferences, 0, nowMs)) return;
    book.metadata.clear();
    book.path = "";
...
void ReaderScreen::draw(ui::Context& ui, const StorageManager& storage, uint32_t nowMs) {
    ...
    if (!ui.redraw({0, 0, ui.width(), ui.height()},
                   frameSignature(before, reader.currentWord(), after, chapterLabel, footer, overlay, progress,
                                  reading, settings, batteryModel))) return;
    ...
    if (!before.isEmpty()) drawText(before, static_cast<int16_t>(x - 24 - textWidth(before)), baseline,
                                          ui.blend(ui::themes::ColorRole::Foreground, 62));
    drawWord(word, x, baseline, focus, ui);
    if (!after.isEmpty()) drawText(after, static_cast<int16_t>(x + wordWidth + 24), baseline,
                                         ui.blend(ui::themes::ColorRole::Foreground, 62));
    ...
    gfx.setCursor(static_cast<int16_t>(ui.width() - 18 - footer.length() * 12), static_cast<int16_t>(ui.height() - 26));
    gfx.print(footer);
    if (!overlay.isEmpty()) {
...
void ReaderScreen::handleTouch(ui::Context& ui, uint32_t nowMs, Preferences& preferences) {
    ...
    if (reader.playing()) {
        ...
        if (doubleTap(touch.x, touch.y, nowMs)) requestPause(preferences, nowMs);
        return;
    }
    ...
    if (touchIntent_ == TouchIntent::Scrub) {
        ...
        scrubSteps_ = steps;
        if (changed) reader.seekRelative(touchStartWord_, steps);
        if (ended) {
            resetTouch();
            book.save(preferences, store, reader, true, nowMs);
        }
    ...
    if (doubleTap(touch.x, touch.y, nowMs)) start(nowMs, true);
}

void ReaderScreen::toggle(Preferences& preferences, uint32_t nowMs) {
    if (reader.playing()) requestPause(preferences, nowMs);
    else start(nowMs, true);
}
...
bool ReaderScreen::shouldFinishPause(uint32_t nowMs) const {
    if (!reader.playing() || !session.pauseAtSentenceEndRequested) return false;
...
int ReaderScreen::scrubSteps(int deltaX) const {
    ...
    if (distance < kSwipeThreshold) return 0;
...
String ReaderScreen::phantomAfter(const ReadingLoop& reader, uint8_t sizeIndex) const {
    if (reader.currentIndex() + 1 >= reader.wordCount()) return "";
...
int ReaderScreen::focusIndex(const String& word) const {
    ...
    for (size_t index = 0; index < word.length();) {
        uint16_t codepoint = 0;
        nextCodepoint(word, index, codepoint);
        if (wordCodepoint(codepoint)) ++characters;
    }
    if (characters == 0) return word.isEmpty() ? -1 : 0;
    ...
    for (size_t index = 0; index < word.length(); ++glyph) {
        uint16_t codepoint = 0;
        nextCodepoint(word, index, codepoint);
        if (wordCodepoint(codepoint) && ordinal++ == target) return glyph;
    }
    return 0;
}
...
int16_t ReaderScreen::textWidth(const String& value) const {
    text_.setFont(font_);
    if (typography_.tracking == 0) return text_.advance(value.c_str());
    int16_t width = 0;
    for (size_t index = 0; index < value.length();) {
        ...
        if (index < value.length()) width = static_cast<int16_t>(width + typography_.tracking);
    }
    return width;
}
...
void ReaderScreen::drawWord(const String& word, int16_t x, int16_t baseline, int focus, ui::Context& ui) {
    ...
    for (size_t index = 0; index < word.length(); ++glyph) {
        uint16_t codepoint = 0;
        nextCodepoint(word, index, codepoint);
        text_.setColors(typography_.focusHighlight && glyph == focus
...
void ReaderScreen::drawBattery(ui::Context& ui) {
    if (battery_.percent == 0 && battery_.voltage <= 0) return;
    Arduino_GFX& gfx = ui.gfx();
    ...
    if (fill > 0) gfx.fillRect(static_cast<int16_t>(x + 2), 12, fill, 9, ui.color(ui::themes::ColorRole::Accent));
    gfx.setFont(static_cast<const GFXfont*>(nullptr));
    gfx.setTextSize(2);
    gfx.setTextColor(ui.color(ui::themes::ColorRole::Muted));
