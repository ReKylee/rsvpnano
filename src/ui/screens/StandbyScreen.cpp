...
void StandbyScreen::begin(ui::Context& ui, uint32_t nowMs, size_t bookIndex, size_t wordIndex) {
    columns_ = std::clamp<uint16_t>(ceilDiv(std::max<int16_t>(1, ui.width()), kCellSize), 1,
                                    standby::kMaxStandbyColumns);
    rows_ = std::clamp<uint16_t>(ceilDiv(std::max<int16_t>(1, ui.height()), kCellSize), 1,
                                 standby::kMaxStandbyRows);
    screensaver_.select(standby::Kind::Life, columns_, rows_);
    const uint32_t seed = nowMs ^ (static_cast<uint32_t>(bookIndex) << 16U)
                        ^ (static_cast<uint32_t>(wordIndex) * 2654435761UL);
...
void StandbyScreen::update(ui::Context& ui, uint32_t nowMs) {
    if (!screensaver_) begin(ui, nowMs, 0, 0);
    if (static_cast<int32_t>(nowMs - nextFrameMs_) < 0) return;
    uint8_t steps = 0;
    do {
        screensaver_.step();
        ...
        for (uint16_t column = 0; column < columns_; ++column) {
            ...
            const uint16_t color = standby::cellAlive(frame.cells, index)
                ...
                : frame.dimCells.valid() && standby::cellAlive(frame.dimCells, index)
                    ? dim : ui.color(ui::themes::ColorRole::Background);
