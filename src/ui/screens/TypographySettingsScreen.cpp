...
namespace screens {
    ...
    void typographySettings(ui::Context& ui, ReaderSettings& config, FontCatalog& fonts, Preferences& preferences,
        ...
        ui::Column font{{content.x, static_cast<int16_t>(content.y + 30), columnWidth,
                         static_cast<int16_t>(content.h - 30)}, 4};
        ui::Column geometry{{static_cast<int16_t>(content.x + columnWidth + gap),
                             static_cast<int16_t>(content.y + 30), columnWidth,
                             static_cast<int16_t>(content.h - 30)}, 4};

        if (ui.setting(font.next(27), ui.text(UiText::FontSize), FontCatalog::sizeLabel(config.fontSizeIndex))) {
        ...
        if (ui.setting(font.next(27), ui.text(UiText::Typeface), fonts.typefaceLabel(config.typefaceIndex))) {
        ...
        if (ui.toggle(font.next(27), "Focus letter", config.typography.focusHighlight)) {
            config.typography.focusHighlight = settings::toggle<pref::TypographyFocusHighlight>(preferences);
        }

        if (const auto value = ui.slider(geometry.next(27), "Tracking", config.typography.tracking, -2, 3, 1, " px");
            value.changed) {
        ...
        if (const auto value = ui.slider(geometry.next(27), "Anchor", config.typography.anchor, 30, 40, 1, "%");
            value.changed) {
        ...
        if (const auto value =
                ui.slider(geometry.next(27), "Guide width", config.typography.guideWidth, 12, 30, 2, " px");
            value.changed) {
        ...
        if (const auto value = ui.slider(geometry.next(27), "Guide gap", config.typography.guideGap, 2, 8, 1, " px");
            value.changed) {
        ...
        if (ui.button(font.next(27), "Reset typography")) {
